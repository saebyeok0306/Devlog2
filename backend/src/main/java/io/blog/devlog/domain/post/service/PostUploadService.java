...
public class PostUploadService {
    ...
    public Post savePost(RequestPostDto requestPostDto) {
        ...
        if (publicPostService.isPubliclyVisible(post)) {
            sitemapService.addPostToSubSitemap(PostUrlDto.of(requestPostDto.getCategoryId(), requestPostDto.getUrl()));
        }
        return post;
    }
    ...
    public Post editPost(Post prevPost, RequestEditPostDto requestEditPostDto) {
        ...
        if (!prevPostUrl.equals(post.getUrl()) || !prevCategoryId.equals(post.getCategory().getId())) {
            sitemapService.deletePostFromSubSitemap(PostUrlDto.of(prevCategoryId, prevPostUrl));
            if (publicPostService.isPubliclyVisible(post)) {
                sitemapService.addPostToSubSitemap(PostUrlDto.of(post.getCategory().getId(), post.getUrl()));
            }
        }
        return post;
    }
